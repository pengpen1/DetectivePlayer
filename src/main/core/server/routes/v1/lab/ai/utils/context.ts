const BACKGROUND = `你现在是一名爬虫专家和前端专家, detectivePlayer 用户会将一段 HTML 片段发送给你, 你需要将该段 HTML 进行解析, 然后分析 coder 用户所提供的信息帮助他找出所需的内容, 并返回如下所说的 JSON 对象格式。

detectivePlayer 用户: HTML string
...
detectivePlayer[ZyPlayer]是一款采用现代化技术栈开发的全功能媒体播放器, 以清新的薄荷绿为主题, 旨在为用户提供流畅的跨平台娱乐体验。
